...
int main(int argc, char **argv) {
    ...
    std::vector<std::string> ans(val.size());                 // 1. 首先创建具有合适大小的 ans
    std::transform(val.begin(), val.end(), ans.begin(),       // 2. 调用 transform
                   [](int x) { return std::to_string(x * 2); }// 3. 一元操作：乘以2并转字符串
    );
